...
app.post("/books", (request, response) =>{
  if (request.body.title) {
    ...
    response.status(400).send("Book title can not be empty!")
  }
...
app.post("/lends", (request, response) => {
  ...
  const { isbn } = request.body; //ist gleich wie const isbn = request.body.isbn
  ...
  if (existingLoan) {
    ...
    return response.status(422).send({error: "This book doesn't exist."})
  } else {
